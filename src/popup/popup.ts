...
const websiteButtons = websites.map((website) => {
  return new WebsiteButton(website);
});

const websiteButtonContainer = document.querySelector(
...
chrome.storage.sync.onChanged.addListener((_) => {
  setInformationText(
    'Settings changed, update the website to see the changes.'
  );
});
